...
namespace GOST_34._10_18
{
    public class Streebog
    {
        ...
        public Streebog(int outputLenght)
        {
            if (outputLenght == 512)
            {
                for (int i = 0; i < 64; i++)
        ...
        private byte[] AddModule512(byte[] a, byte[] b)
        {
        ...
        public byte[] GetHash(byte[] message)
        {
            ...
            while (len >= 512)
            {
                ...
                h = G_n(N, h, tempMes);
                N = AddModule512(N, N_512.Reverse().ToArray());
                Sigma = AddModule512(Sigma, tempMes);
                len -= 512;
            }
            ...
            N = AddModule512(N, MesLen.Reverse().ToArray());
            Sigma = AddModule512(Sigma, paddedMes);
            h = G_n(N_0, h, N);
            h = G_n(N_0, h, Sigma);
            if (outLen == 512)
                return h;
